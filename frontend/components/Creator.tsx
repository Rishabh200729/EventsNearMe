...
export default function Creator({ user, addOptimisticEvent }: { user: user, addOptimisticEvent?: (e: any) => void }) {
        ...
        <div className="glass-card max-w-2xl mx-auto">
            ...
            <form className="space-y-6" ref={ref} action={async (formData) => {
                if (addOptimisticEvent) {
                    addOptimisticEvent({
                        _id: "optimistic-" + Math.random().toString(),
                        title: formData.get("name") as string,
                        description: formData.get("desc") as string,
                        date: formData.get("date") as string,
                        category: (formData.get("category") as string)?.toLowerCase(),
                        organizerId: { firstName: "You", lastName: "(Publishing...)" },
                        pending: true
                    });
                }
                ...
                <SubmitButton type="submit" loadingText="Launching..." className="premium-button w-full py-4 text-lg">
                    Launch Event
                </SubmitButton>
            </form>
        </div>
